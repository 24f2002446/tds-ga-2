...
def get_p95_lat(all_lats):
    ...
    if index.is_integer():
        ...
        return lower_value + (upper_value - lower_value) * (index - int(math.floor(index)))
...
async def post(data: Data):
    ...
    for region in item.regions:
        ...
        matches.sort(key=lambda x: x.latency_ms)

        avg_latency = sum([obj.get("latency_ms") for obj in matches]) / len(matches)
        p95_latency = get_p95_lat(list(map(lambda x: x.get("latency_ms"), list(matches))))
        avg_uptime = sum([obj.get("uptime_pct") for obj in matches]) / len(matches)
        breaches = len([obj for obj in matches if obj.get("latency_ms") > item.threshold_ms])

        res = {"region": region, "avg_latency": avg_latency, "p95_latency": p95_latency, "avg_uptime": avg_uptime, "breaches": breaches}
        result.append(res)
